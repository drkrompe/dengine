...
const onMouseMove = (event) => {
    ...
    // X [-2, 2] left->right
    // Y [1, -1] top->bottom
    mouse.x = ((event.clientX - canvasData.left) / canvasData.width) * 4 - 2 + Camera.camera.position.x;
    mouse.y = -1 * ((event.clientY - canvasData.top) / canvasData.height) * 2 + 1 + Camera.camera.position.y;
